...
class RAGPipeline:
    ...
    def query(self, question: str, k: int = 3, initial_k: int = 20) -> List[Document]:
        ...
        if self.use_pinecone and self._pinecone_available:
            ...
            if results:
                # Serialize to dicts before caching (Documents are not JSON-serializable)
                if redis and redis.is_available():
                    try:
                        serializable = [{"page_content": d.page_content, "metadata": d.metadata} for d in results]
                        redis.set_rag_results(question, serializable)
                    except Exception as e:
                        print(f"[RAG] Redis write error: {e}")
                return results
        ...
        # Serialize to dicts before caching
