...
async def chat_endpoint(request):
    ...
    try:
        ...
        error_str = str(e)
        print(f"[{session_id}] Error processing request: {error_str}")

        if "rate_limit" in error_str.lower() or "429" in error_str or "413" in error_str or "quota" in error_str.lower():
            friendly_error = "I'm currently experiencing high demand. Please wait a moment and try again."
        elif "session" in error_str.lower() or "not found" in error_str.lower():
            friendly_error = "Your session could not be found. Please start a new chat."
        else:
            friendly_error = "Something went wrong on my end. Please try again shortly."
